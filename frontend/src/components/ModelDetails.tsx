...
export function ModelDetails() {
  ...
  const isModerated = info?.top_provider?.is_moderated
    ...
    <section className="rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 p-4 shadow">
      ...
      <div className="text-sm text-gray-700 dark:text-gray-200">
        ...
        {typeof contextLen === 'number' && (
          <div className="text-sm">Context: {formatNumber(contextLen)} tokens{typeof maxComp === 'number' ? ` · max completion ${formatNumber(maxComp)}` : ''}</div>
        )}
        {(pricingPrompt || pricingCompletion) && (
          <div className="text-sm">Pricing: {pricingPrompt ? `prompt ${formatPrice(pricingPrompt)}` : ''}{pricingPrompt && pricingCompletion ? ' · ' : ''}{pricingCompletion ? `completion ${formatPrice(pricingCompletion)}` : ''}</div>
        )}
        {info?.description && (
        ...
        {showAdvanced && (
          <div className="mt-3 space-y-3">
            ...
            <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1">
              {architecture && (
                <div><span className="font-medium text-gray-600 dark:text-gray-300">Architecture</span>: {architecture.modality} {architecture.tokenizer ? `(${architecture.tokenizer})` : ''}</div>
              )}
              {isModerated !== undefined && (
                <div><span className="font-medium text-gray-600 dark:text-gray-300">Moderated</span>: {isModerated ? 'Yes' : 'No'}</div>
              )}
